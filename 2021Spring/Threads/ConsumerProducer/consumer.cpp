...
void cConsumer::operator() () {
    ...
    for (int cycle = 0; cycle < nConsumerCycles; cycle++) {
        ...
        if (pSchedSupport->consOwned_Prod2ConsBuffer_isFresh) {
            {
                // acquire lock and use the content of the consumer-owned transfer buffer
        ...
        // if it's the case, it's important at this point to let the producer know that this 
        // is the last consumer cycle; this is important otherwise the producer will hang 
        // waiting for communication swith the consumer
        ...
        if (!pSchedSupport->prodOwned_Cons2ProdBuffer_isFresh) {
            // acquire lock and refresh the work order for the producer
            {
                ...
                memcpy(pProducerOwned_TransfBuffer, input4Producer, N_INPUT_ITEMS * sizeof(int));
            }
